...
namespace ImGui
{
    ...
    IMGUI_API void          SubTabButton(const char* label, int* currenttab, int number, int total);
    IMGUI_API void          TabButton(const char* label, int* currenttab, int number, int total);
    IMGUI_API bool          LoadingAnimation(ImVec2 pos, const char* label, float radius, int thickness, const ImU32& color);
    IMGUI_API bool          CustomButtonImage(const char* label, const char* desc, const ImVec2& size, bool is_loading, ImTextureID user_texture_id, int index, int variance = 0);
    IMGUI_API bool          CustomButton(const char* label, const ImVec2& size);
    IMGUI_API bool          SelectableWithPopUp(const char* label, bool selected = false, int index = 0, int variance = 0, int list_var = 0); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_ListBox = 1 << 30   // Use for do listbox
