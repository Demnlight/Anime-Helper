...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(34, 34, 41, 255), 5);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(47, 47, 53, 255), 5);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
    {
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.8f + 1.0f), ImColor(34, 34, 40), 36);
        window->DrawList->AddCircle(center, ImMax(2.0f, g.FontSize * 0.8f + 1.0f), ImColor(49, 49, 55), 36);
    }
    float cross_extent = g.FontSize * 0.75f * 0.7071f - 1.0f;
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    ImRect bb = ImRect(bb_frame.Min - ImVec2(3, 0), bb_frame.Max);
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, grab_col, 5/*, ImDrawCornerFlags_Right*/);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, window->Size.x - 20, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(34, 34, 41, 255), 5);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(47, 47, 53, 255), 5);
    }
    ...
    if (label_size.x > 0 && strlen(buf_display) == 0)
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), ImColor(150, 150, 150, 255), label);
...
bool ImGui::CustomButtonImage(const char* label, const char* desc, const ImVec2& size, bool is_loading, ImTextureID user_texture_id, int index, int variance)
{
    ...
    const ImVec4 hover_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, g.Style.Alpha);
    const ImVec4 hover_dis = ImVec4(150 / 255.f, 150 / 255.f, 150 / 255.f, g.Style.Alpha);
    ...
    const ImVec4 hover_actr = ImVec4(100 / 255.f, 100 / 255.f, 255 / 255.f, g.Style.Alpha);
    const ImVec4 hover_disr = ImVec4(49 / 255.f, 49 / 255.f, 55 / 255.f, g.Style.Alpha);
    static std::map<ImGuiID, ImVec4> custom_button_rect_hover_color;

    auto it_hcolorr = custom_button_rect_hover_color.find(id);
    if (it_hcolorr == custom_button_rect_hover_color.end())
    {
        custom_button_rect_hover_color.insert({ id, hover_disr });
        it_hcolorr = custom_button_rect_hover_color.find(id);
    }
    if (hovered || pressed)
    {
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(33, 33, 40, 255), 5);
    window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(it_hcolorr->second), 5);

    auto label_size = CalcTextSize(label);

    if (!is_loading) {
        window->DrawList->AddImage(user_texture_id, bb.Min + ImVec2(2, 2), bb.Max - ImVec2(2, 30), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255));

    }
    else {
        ImVec2 animationStartPos = ImVec2(bb.Min.x + (size.x / 2), bb.Min.y + (size.y / 2));
        LoadingAnimation(animationStartPos, "", 15.f, 4, ImColor(49, 156, 255));
    }
    window->DrawList->AddText(GetIO().Fonts->Fonts[2], 20.f, ImVec2(bb.Min.x + 5, bb.Max.y - 25), ImColor(255, 255, 255, 255), ClampName(label, 25).c_str());
    //window->DrawList->AddText(ImVec2(bb.Min.x + 5, bb.Max.y - 35 + 2 + label_size.y), ImGui::GetColorU32(it_hcolor->second), ClampName(desc, 23).c_str());

    //window->DrawList->AddText(ImVec2(bb.Min.x + size.x / 2 - label_size.x / 2, bb.Min.y + size.y / 2 - label_size.y / 2), ImGui::GetColorU32(it_hcolor->second), txt);
    if (variance == 0)
    {
        if (GetIO().MouseDown[1] && hovered)
        {
        ...
        if (ImGui::BeginPopup(label))
        {
            if (ImGui::Selectable(Xorstr("Add to my anime list")))
            {
                ...
                if (g_Globals->AnimeFavorites.size() < 1)
                    add = true;
...
bool ImGui::CustomButton(const char* label, const ImVec2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    auto label_size = CalcTextSize(label);
    window->DrawList->AddText(GetIO().Fonts->Fonts[2], 20.f, ImVec2(bb.Min.x + size.x / 2 - label_size.x / 2, bb.Min.y + size.y / 2 - label_size.y / 2), ImColor(255, 255, 255, 255), label);
...
void ImGui::TabButton(const char* label, int* selected, int num, int total) {
    ...
    const ImVec4 hovered_circle_active  = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 255 / 255.f);
    const ImVec4 hovered_circle_disable = ImVec4(150 / 255.f, 150 / 255.f, 150 / 255.f, 255 / 255.f);

    static std::map<ImGuiID, ImVec4> tab_button_hover_color;
    auto it_hcolor = tab_button_hover_color.find(id);
    if (it_hcolor == tab_button_hover_color.end())
    {
        tab_button_hover_color.insert({ id, hovered_circle_active });
        it_hcolor = tab_button_hover_color.find(id);
    }
    ...
    if (*selected == num)
    {
        window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y + 3), ImVec2(pos.x + 130, pos.y + size.y - 3), ImColor(34, 34, 41), 3.f);
        window->DrawList->AddRect(ImVec2(pos.x, pos.y + 3), ImVec2(pos.x + 130, pos.y + size.y - 3), ImColor(47, 47, 53), 3.f);
    }
    else
    {
        //if (hovered)
        //    window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y + 3), ImVec2(pos.x + 130, pos.y + size.y - 3), ImColor(30, 30, 30), 3.f);
    }

    PushFont(ImGui::GetIO().Fonts->Fonts[2]);
    window->DrawList->AddText(ImVec2(pos.x + 5, pos.y + size.y / 2 - label_size.y / 2 - 4), ImGui::GetColorU32(it_hcolor->second), label);
    PopFont();
}
...
bool ImGui::SelectableWithPopUp(const char* label, bool selected, int index, int variance, int list_var)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    //RenderTextClipped(text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    window->DrawList->AddText(ImVec2(pos.x, pos.y), ImGui::GetColorU32(it_hcolor->second), label);
    // Automatically close popups
    /*if (pressed && (window->Flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiSelectableFlags_DontClosePopups) && !(window->DC.ItemFlags & ImGuiItemFlags_SelectableDontClosePopup))
        CloseCurrentPopup();*/

    /*if (GetIO().MouseDown[1] && selected && hovered)
    {
        ImGui::OpenPopup(label);
    }*/
    ...
    /*if (ImGui::BeginPopup(label))
    {
        ...
        {
        case 0:
            if (ImGui::Selectable(Xorstr("Move to finished")))
            {
                static bool add = true;
                add = true;

                for (int i = 0; i < g_Globals->anime_list_finished.size(); i++)
                    if (strcmp(g_Globals->anime_list_finished.at(i).name.c_str(), label) == 0)
                        add = false;

                if (g_Globals->anime_list_finished.size() < 1)
                    add = true;
            ...
            if (ImGui::Selectable(Xorstr("Move to watching")))
            {
                static bool add = true;
                add = true;

                for (int i = 0; i < g_Globals->anime_list_watching.size(); i++)
                    if (strcmp(g_Globals->anime_list_watching.at(i).name.c_str(), label) == 0)
                        add = false;

                if (g_Globals->anime_list_watching.size() < 1)
                    add = true;
            ...
            if (ImGui::Selectable(Xorstr("Move to wanna start")))
            {
                static bool add = true;
                add = true;

                for (int i = 0; i < g_Globals->anime_list_wannastart.size(); i++)
                    if (strcmp(g_Globals->anime_list_wannastart.at(i).name.c_str(), label) == 0)
                        add = false;

                if (g_Globals->anime_list_wannastart.size() < 1)
                    add = true;
        ...
        case 1:
            ...
            {
                if (list_var == 0)
                    g_Globals->anime_list_finished.erase(g_Globals->anime_list_finished.begin() + index);
                else if (list_var == 1)
                    g_Globals->anime_list_watching.erase(g_Globals->anime_list_watching.begin() + index);
                else if (list_var == 2)
                    g_Globals->anime_list_wannastart.erase(g_Globals->anime_list_wannastart.begin() + index);
                ImGui::CloseCurrentPopup();
            }
            if (ImGui::Selectable(Xorstr("Add to favorite")))
            {
                static bool add = true;
                add = true;
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 10), window->DC.CursorPos + frame_size);
    ImRect bb(frame_bb.Min, frame_bb.Max + ImVec2(0.0f, 0.0f));
    ...
    BeginGroup();
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x + size.x / 2 - label_size.x / 2, frame_bb.Min.y - label_size.y - 15), label);

    BeginChildFrame(id, frame_bb.GetSize(), ImGuiWindowFlags_ListBox);
    return true;
}
